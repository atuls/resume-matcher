/**
 * Fix Empty Parsed Fields Script - SQL Version
 * 
 * This script uses direct SQL to identify and fix analysis results with empty parsed fields
 * but that have data in the raw_response.
 */
...
async function fixEmptyParsedFields() {
  ...
  try {
    ...
    // Find records that have raw_response but empty parsed fields
    const incompleteRecordsQuery = `
      ...
      AND (
        ...
        parsed_red_flags IS NULL OR 
        parsed_summary IS NULL OR 
        parsed_work_history = '[]' OR 
        parsed_red_flags = '[]' OR 
        parsed_summary = ''
      )
      LIMIT 100
